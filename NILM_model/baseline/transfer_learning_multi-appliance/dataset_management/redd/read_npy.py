...
file_path = 'total/train_set2.npy'
...
for i in range(9):  # 一共有16个子图
    ...
    for j in range(2):
        if appliance2[j] == 'aggregate':
            data_to_plot = test_set_copy[count_values[channel][i], :, j]  # 假设你想绘制第一个数据集的第一列数据
            axes[row, col].plot(data_to_plot, label=f'{appliance2[j]}', linestyle='--', color='gray')
        else:
            data_to_plot = test_set_copy[count_values[channel][i], :, channel + 1]  # 假设你想绘制第一个数据集的第一列数据
